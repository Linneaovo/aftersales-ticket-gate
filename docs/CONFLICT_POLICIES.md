...
| POL-GROUND-02 | hard reject | RAG `blocked`（ACL/注入/闲聊） | 是（**冲突题 ungrounded 除外→GROUND-01 软化**） |
...
- **GROUND-01 + 冲突题**：Live RAG 可能 `grounded=false` 或 `blocked+ungrounded`；冲突意图下改为 `force_hitl`，避免硬拒抢走站长人确。ACL/注入（GROUND-02 安全类）仍硬拒。
...
```bash
python scripts/smoke_with_rag.py --compare-live   # 演示前 regenerate；P8 为主路径
pytest tests/test_core.py -k "conflict_blocked or conflict_ungrounded" -q
pytest tests/test_integration_live.py -m integration -k "p8"
```
